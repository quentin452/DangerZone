...
public class MyMLPBuffalo extends EntityMob {
    ...
    public MyMLPBuffalo(final World par1World) {
        super(par1World);
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ...
        for (int i = 1 + this.worldObj.rand.nextInt(5), var4 = 0; var4 < i; ++var4) {
            ...
            switch (var5) {
                case 0: {
                    final ItemStack is = this.dropItemRand(Items.apple, 1);
                    break;
                }
            }
        }
                ...
                if (this.getDistanceSqToEntity((Entity) e) < (3.8f + e.width / 2.0f) * (3.8f + e.width / 2.0f)) {
                    ...
                    if (this.worldObj.rand.nextInt(3) == 0 || this.worldObj.rand.nextInt(5) == 1) {
                        ...
                        if (!this.worldObj.isRemote) {
                            if (this.worldObj.rand.nextInt(3) == 1) {
                                this.worldObj.playSoundAtEntity((Entity) e, "random.explode", 1.4f, 1.0f);
                            } else {
                                this.worldObj.playSoundAtEntity((Entity) e, "random.explode", 1.0f, 1.0f);
                            }
                        }
                    }
                } else {
    ...
    private boolean isSuitableTarget(final EntityLivingBase par1EntityLiving, final boolean par2) {
        ...
        if (par1EntityLiving instanceof MyThunderHooves) {
            return false;
        }
        if (par1EntityLiving instanceof EntityCreeper) {
            return false;
        }
        if (par1EntityLiving instanceof MyMLPBuffalo) {
            return false;
        }
    ...
    public static class RenderTheMob11 extends RenderLiving {

        private static final ResourceLocation buffalotexture;

        public RenderTheMob11(final ModelBase par1ModelBase, final float par2) {
            super(par1ModelBase, par2);
        }

        protected ResourceLocation getEntityTexture(final Entity entity) {
            return RenderTheMob11.buffalotexture;
        }

        static {
            buffalotexture = new ResourceLocation(Tags.MODID + ":" + "textures/entity/mlpbuffalo.png");
        }
    }
}
