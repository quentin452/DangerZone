...
public class AlosaurusInstance extends DungeonMobInstance {
    ...
    public AlosaurusInstance(World par1World) {
        super(par1World);
        this.experienceValue = 15;
        this.ignoreHeight = false;
        this.setSize(1.2F, 1.5F);

        if (this.worldObj != null) this.stoneChance = 6 * Helper.getDifficulty(this.worldObj);
        else this.stoneChance = 15;
    ...
    public boolean getCanSpawnHere() {
        ...
        if (this.posY > 64.0D && !this.ignoreHeight) return false;
    ...
    public boolean attackEntityAsMob(Entity ent) {
        ...
        if (moo < this.stoneChance && ent instanceof EntityPlayer
    ...
    public void onEntityUpdate() {
        ...
        if (this.incoming && !this.worldObj.isRemote) {
            if (this.waitTick) this.waitTick = false;
            else {
