...
import fr.iamacat.dangerzone_iamacatfr.worldgen.FairyGroupGenerator;
...
public class FairyInstance extends EntityAnimal {
    ...
    public FairyFishHookInstance fishEntity;
    ...
    public FairyInstance(World world) {
        super(world);
    ...
    protected void entityInit() {
        super.entityInit();
        dataWatcher.addObject(B_FLAGS, 0);
        dataWatcher.addObject(B_FLAGS2, 0);
        dataWatcher.addObject(B_TYPE, 0);
        dataWatcher.addObject(B_HEALTH, 0);
        dataWatcher.addObject(B_NAME_ORIG, 0);
        dataWatcher.addObject(S_OWNER, "");
        dataWatcher.addObject(S_NAME_REAL, "");
        dataWatcher.addObject(I_TOOL, 0);
    }
    ...
    private void _dump_() {
        ...
        sb.append(" : ");
        int b0 = dataWatcher.getWatchableObjectInt(B_TYPE);
        int b1 = dataWatcher.getWatchableObjectInt(B_FLAGS);
        int b2 = dataWatcher.getWatchableObjectInt(B_FLAGS2);

        sb.append(
            String.format("%8s", Integer.toBinaryString(b0))
                .replace(' ', '0'));
        sb.append("-");
        sb.append(
            String.format("%8s", Integer.toBinaryString(b1))
                .replace(' ', '0'));
        sb.append("-");
        sb.append(
            String.format("%8s", Integer.toBinaryString(b2))
                .replace(' ', '0'));
    ...
    public void writeEntityToNBT(NBTTagCompound nbt) {
        ...
        nbt.setInteger("flags", dataWatcher.getWatchableObjectInt(B_FLAGS));
        nbt.setInteger("flags2", dataWatcher.getWatchableObjectInt(B_FLAGS2));
        nbt.setInteger("type", dataWatcher.getWatchableObjectInt(B_TYPE));
        nbt.setInteger("nameOrig", dataWatcher.getWatchableObjectInt(B_NAME_ORIG));
    ...
    public void readEntityFromNBT(NBTTagCompound nbt) {
        ...
        dataWatcher.updateObject(B_FLAGS, nbt.getInteger("flags"));
        dataWatcher.updateObject(B_FLAGS2, nbt.getInteger("flags2"));
        dataWatcher.updateObject(B_TYPE, nbt.getInteger("type"));
        dataWatcher.updateObject(B_NAME_ORIG, nbt.getInteger("nameOrig"));
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(DangerZoneConfig.GENERAL_HEALTH_BASE);
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(DangerZoneConfig.GENERAL_SPEED_BASE);
    ...
    public void onUpdate() {
        ...
        if (this.createGroup) {
            ...
            final FairyGroupGenerator group = new FairyGroupGenerator(
                DangerZoneConfig.SPAWN_FACTION_MIN_SIZE,
                DangerZoneConfig.SPAWN_FACTION_MAX_SIZE,
                getFaction());
            if (group.generate(worldObj, rand, i, j, k)) {
                ...
                if (!worldObj.isRemote) {
                    DangerZone.packetProxy.sendFairyDespawn(this);
                }
        ...
        if (scout()) {
            this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .setBaseValue(DangerZoneConfig.GENERAL_SPEED_SCOUT);
        } else {
            this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .setBaseValue(DangerZoneConfig.GENERAL_SPEED_BASE);
        }
        ...
        if (withered()) {
            ...
            speed.setBaseValue(speed.getAttributeValue() * DangerZoneConfig.GENERAL_SPEED_WITHER_MULT);
        }
        ...
        if (getHealth() > 0.0F) {
            ...
            if (flymode()) {
                if (!liftOff() && ridingEntity != null
                    && !ridingEntity.onGround
                    && ridingEntity instanceof EntityLivingBase) {
                    ridingEntity.fallDistance = 0F;

                    if (ridingEntity.motionY < DangerZoneConfig.DEF_FLOAT_RATE) {
                        ridingEntity.motionY = DangerZoneConfig.DEF_FLOAT_RATE;
                    }

                    final boolean isJumping = ReflectionHelper
                        .getPrivateValue(EntityLivingBase.class, (EntityLivingBase) ridingEntity, MCP_ISJUMPING);
                    if (isJumping && ridingEntity.motionY < DangerZoneConfig.DEF_FLAP_RATE && canFlap()) {
                        ridingEntity.motionY = DangerZoneConfig.DEF_FLAP_RATE;
                    }
                } else {
                    if (motionY < DangerZoneConfig.DEF_FLOAT_RATE) {
                        motionY = DangerZoneConfig.DEF_FLOAT_RATE;
                    }

                    if (canFlap() && checkGroundBelow() && motionY < 0D) {
                        motionY = DangerZoneConfig.DEF_FLOAT_RATE * DangerZoneConfig.DEF_SOLO_FLAP_MULT;
                    }

                    if (liftOff() && ridingEntity != null) {
                        ridingEntity.fallDistance = 0F;
                        motionY = ridingEntity.motionY = DangerZoneConfig.DEF_FLAP_RATE
                            * DangerZoneConfig.DEF_LIFTOFF_MULT;
                    }
                }
            }
            ...
            if (worldObj.isRemote) {
                if (++particleCount >= DangerZoneConfig.DEF_MAX_PARTICLES) {
                    particleCount = rand.nextInt(DangerZoneConfig.DEF_MAX_PARTICLES >> 1);
                ...
                if (nameEnabled() && tamed()) {
                    if (!rulerName().equals("")) {
                        DangerLogger.LOGGER.info("EntityFairy.onUpdate: calling proxy.openRenameGUI");
                        DangerZone.packetProxy.openRenameGUI(this);
                    } else {
                        DangerLogger.LOGGER.info("EntityFairy.onUpdate: tame but no ruler...");
                    }
                }
            }
    ...
    public void despawnFollowers() {
        if (queen() && getFaction() > 0) {
            List list = worldObj.getEntitiesWithinAABB(FairyInstance.class, boundingBox.expand(40D, 40D, 40D));

            for (int j = 0; j < list.size(); j++) {
                FairyInstance fairy = (FairyInstance) list.get(j);
    ...
    public void updateEntityActionState() {
        ...
        if (flymode()) {
            ...
            if (getFlyTime() <= 0 || (flyBlocked
                && (ridingEntity == null || (entityToAttack != null && ridingEntity == entityToAttack)))) {
                setCanFlap(false);
            } else {
                setCanFlap(true);
            }
        ...
        if (worldObj.difficultySetting == EnumDifficulty.PEACEFUL && entityToAttack != null
    ...
    public PathEntity roam(Entity target, Entity actor, float griniscule) {
        ...
        for (int q = 0; q < MAX_PATHING_TRIES; q++) {
            ...
            if (j > 4 && j < worldObj.getHeight() - 1 && isAirySpace(i, j, k) && !isAirySpace(i, j - 1, k)) {
                PathEntity dogs = worldObj
                    .getEntityPathToXYZ(actor, i, j, k, DangerZoneConfig.BEHAVIOR_PATH_RANGE, false, false, true, true);
    ...
    private void teleportToRuler(EntityLivingBase entity) {
        ...
        for (int l = 0; l <= 4; l++) {
            for (int i1 = 0; i1 <= 4; i1++) {
                if ((l < 1 || i1 < 1 || l > 3 || i1 > 3) && worldObj.getBlock(i + l, k - 1, j + i1)
                    .isNormalCube()
                    && !worldObj.getBlock(i + l, k, j + i1)
                        .isNormalCube()
    ...
    private void handleAnger() {
        ...
        if (entityToAttack != null) {
            ...
            if (enemy_dist >= DangerZoneConfig.BEHAVIOR_PURSUE_RANGE
                || (rand.nextBoolean() && !canEntityBeSeen(entityToAttack))) {
                ++loseInterest;

                if (loseInterest
                    >= (tamed() ? DangerZoneConfig.BEHAVIOR_AGGRO_TIMER : DangerZoneConfig.BEHAVIOR_AGGRO_TIMER * 3)) {
                    setTarget(null);
                    loseInterest = 0;
                }
            } else {
            ...
            if (guard() && getFaction() > 0 && ruler != null && ruler instanceof FairyInstance) {
                FairyInstance fairy = (FairyInstance) ruler;

                if (fairy.entityToAttack != null) {
                    float queen_dist = getDistanceToEntity(fairy);

                    if (queen_dist < DangerZoneConfig.BEHAVIOR_DEFEND_RANGE
                        && enemy_dist < DangerZoneConfig.BEHAVIOR_DEFEND_RANGE
    ...
    private void handleFear() {
        if (getEntityFear() != null) {
            if (getEntityFear().isDead) {
                ...
                if (dist < DangerZoneConfig.BEHAVIOR_FEAR_RANGE) {
    ...
    private void handleRuler() {
        ...
        if (ruler == null) {
            ...
            if (!tamed() && !queen()) {
                ...
                List list = worldObj.getEntitiesWithinAABB(FairyInstance.class, boundingBox.expand(d, d, d));
        ...
        if (ruler != null && !hasPath() && !posted()) {
            ...
            if ((guard() || queen()) && canEntityBeSeen(ruler) && dist > 5F && dist < 16F) {
                ...
                if (scout() && ruler instanceof FairyInstance) {
                    ...
                    } else if (dist > (ruler instanceof FairyInstance ? 12F : 6F)) {
                        PathEntity dest = roam(ruler, this, 0F);
        ...
        if (getFaction() > 0) {
            ...
            } else if (ruler != null && ruler instanceof FairyInstance) {
    ...
    private void handleSocial() {
        ...
        for (int j = 0; j < list.size(); j++) {
            ...
            if (canEntityBeSeen(entity) && !entity.isDead) {
                if ((ruler != null || queen()) && entity instanceof FairyInstance && sameTeam((FairyInstance) entity)) {
                    FairyInstance fairy = (FairyInstance) list.get(j);
    ...
    private void handleHealing() {
        ...
        if (entityHeal != null) {
            if (entityHeal.getHealth() <= 0 || entityHeal.isDead) {
                ...
                PathEntity dest = worldObj.getPathEntityToEntity(this, entityHeal, 16F, false, false, true, true);
        ...
        if (entityHeal == null && healTime <= 0) {
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                if (canEntityBeSeen(entity) && !entity.isDead) {
                    if (entity instanceof FairyInstance) {
                        FairyInstance fairy = (FairyInstance) list.get(j);

                        if (fairy.getHealth() > 0 && sameTeam(fairy) && fairy.getHealth() < fairy.getMaxHealth()) {
                            this.entityHeal = fairy;
                            PathEntity dest = worldObj
                                .getPathEntityToEntity(this, entityHeal, 16F, false, false, true, true);
    ...
    private void doHeal(EntityLivingBase guy) {
        ...
        try {
            final Method onImpact = ReflectionHelper
                .findMethod(EntityPotion.class, potion, MCP_ONIMPACT, MovingObjectPosition.class);
    ...
    public void castRod() {
        if (fishEntity != null) {
            ...
            FairyFishHookInstance hook = new FairyFishHookInstance(worldObj, this);
    ...
    public int getSkin() {
        return dataWatcher.getWatchableObjectInt(B_TYPE) & 0x03;
    }
    ...
    protected void setSkin(int skin) {
        ...
        int int0 = dataWatcher.getWatchableObjectInt(B_TYPE);
        int0 = int0 & 0xfc;
        int0 = int0 | skin;

        dataWatcher.updateObject(B_TYPE, int0);
    }

    public int getJob() {
        return (dataWatcher.getWatchableObjectInt(B_TYPE) >> 2) & 0x03;
    }
    ...
    protected int getFaction() {
        return (dataWatcher.getWatchableObjectInt(B_TYPE) >> 4) & 0x0f;
    }
    ...
    public void setFaction(int faction) {
        ...
        dataWatcher.updateObject(B_TYPE, Integer.valueOf(int0));
    }
    ...
    protected void setFairyName(int prefix, int suffix) {
        ...
        int int0 = ((prefix & 0x0f) | ((suffix & 0x0f) << 4));
        dataWatcher.updateObject(B_NAME_ORIG, int0);
    }

    public int getNamePrefix() {
        return dataWatcher.getWatchableObjectInt(B_NAME_ORIG) & 0x0f;
    }

    public int getNameSuffix() {
        return (dataWatcher.getWatchableObjectInt(B_NAME_ORIG) >> 4) & 0x0f;
    }
    ...
    public String getQueenName(int prefix, int suffix, int faction) {
        if (faction < 0 || faction > MAX_FACTION) return Loc.QUEEN + " Error-faction";

        return faction_colors[faction] + Loc.QUEEN + " " + getActualName(prefix, suffix);
    }

    public String getFactionName(int faction) {
        if (faction < 0 || faction > MAX_FACTION) return "Error-faction";
    ...
    public String getDisplayName() {
        if (getFaction() != 0) {
            ...
            if (isRuler(DangerZone.packetProxy.getCurrentPlayer())) {
    ...
    protected boolean getFairyFlag(int object, int offset) {
        return (dataWatcher.getWatchableObjectInt(object) & (1 << offset)) != 0;
    }

    protected void setFairyFlag(int object, int offset, boolean flag) {
        int int0 = dataWatcher.getWatchableObjectInt(object);
        if (flag) {
            int0 |= 1 << offset;
        } else {
            int0 &= ~(1 << offset);
        }
        dataWatcher.updateObject(object, int0);
    }
    ...
    public void setRulerName(String s) {
        if (!s.isEmpty()) DangerLogger.LOGGER.info("setRulerName: " + this.getEntityId() + " = " + s);
    ...
    public void setCustomName(String s) {
        if (!s.isEmpty()) DangerLogger.LOGGER.info("setCustomName: " + this.getEntityId() + " = " + s);
    ...
    /**
     * This is really confusing. The original reads from int 19
     * then writes out to int 22.
     *
    ...
    protected void setFairyHealth(int i) {
        int int0 = dataWatcher.getWatchableObjectInt(B_HEALTH);
        ...
        int0 = (i & 0xff);
        dataWatcher.updateObject(B_HEALTH, int0);
    }

    public int fairyHealth() {
        return dataWatcher.getWatchableObjectInt(B_HEALTH) & 0xff;
    }
    ...
    public boolean isAirySpace(int a, int b, int c) {
        ...
        if (matt == null || matt == Material.air
            || matt == Material.plants
            || matt == Material.vine
            || matt == Material.fire
            || matt == Material.circuits
            || matt == Material.snow) {
            return true;
        }
    ...
    private boolean sameTeam(FairyInstance fairy) {
        if (tamed()) {
            return fairy.tamed() && fairy.getFaction() == 0
                && fairy.rulerName()
                    .equals(this.rulerName());
        ...
        if (!worldObj.isRemote
            ...
            if (isRuler(player)) {
                if (stack != null && getHealth() < getMaxHealth()
                    ...
                    return true;
                } else if (stack != null && ridingEntity == null
                    && !isSitting()
                    ...
                    && stack.stackSize > 0) {
                        ...
                        DangerLogger.LOGGER.info("EntityFairy.interract: consuming paper and setting name enabled");
                        ...
                        if (isSitting()) {
                            ...
                            DangerZone.packetProxy.sendFairyMount(this, player);
                            setFlymode(true);
                            flyTime = 200;
                            setCanFlap(true);
                            return true;
                ...
                if ((getFaction() == 0 || worldObj.difficultySetting == EnumDifficulty.PEACEFUL)
                    && !((queen() || posted()) && tamed())
                    && !crying()
                    && !angry()
                    && stack != null
    ...
    public boolean acceptableFoods(Item i) {
        if (i == Items.speckled_melon) {
            ...
            return i == Items.apple || i == Items.melon || i == Items.sugar || i == Items.cake || i == Items.cookie;
        }
    ...
    private void disband() {
        ...
        if (ruler != null) {
            ...
            if (ruler instanceof EntityPlayer) {
                ...
                DangerZone.packetProxy.sendChat((EntityPlayerMP) ruler, "* §9" + s);
            }
        }
    ...
    public void tameMe(EntityPlayer player) {
        ...
        if (player instanceof EntityPlayerMP) {
            DangerZone.packetProxy.sendChat((EntityPlayerMP) player, "* §9" + s);
        }

        DangerLogger.LOGGER.info("tameMe: " + rulerName() + ": " + this);
    }
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                if (fairy != this && fairy.getHealth() > 0
                    ...
                    if (fairy.ridingEntity != null) {
                        DangerZone.packetProxy.sendFairyMount(fairy, fairy.ridingEntity);
                    }
    ...
    public void alertRuler(Entity entity) {
        if (getFaction() > 0 && ruler != null && ruler instanceof FairyInstance && sameTeam((FairyInstance) ruler)) {
            FairyInstance queen = ((FairyInstance) ruler);
            boolean flag = false;
            List list = worldObj.getEntitiesWithinAABB(FairyInstance.class, queen.boundingBox.expand(40D, 40D, 40D));
            ...
            DangerZone.packetProxy.sendChat((EntityPlayerMP) ruler, "* §c" + s);
        }
    }
    ...
    public void hydraFairy() {
        ...
        setSitting(true);
        onGround = true;
        List list = worldObj.getEntitiesWithinAABB(FairyInstance.class, boundingBox.expand(80D, 80D, 80D));
    ...
    protected void attackEntity(Entity entity, float f) {
        if (attackTime <= 0 && f < (tamed() ? 2.5F : 2.0F)
            && ((entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY)
                || f == 0F)) {
            attackTime = 20;
            ...
            if (flymode() && canFlap()
                ...
                && !(entity instanceof FairyInstance || entity instanceof EntityFlying)) {
                // Scout's Totally Leet Air Attack.
                DangerZone.packetProxy.sendFairyMount(this, entity);
                setFlymode(true);
                flyTime = 200;
                setCanFlap(true);
                attackTime = 35;
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float damage) {
        ...
        if (flag && getHealth() > 0) {
            ...
            if (ridingEntity != null && rand.nextInt(2) == 0) {
                DangerZone.packetProxy.sendFairyMount(this, ridingEntity);
            }
        } else if (flag) {
            if (ridingEntity != null) {
                DangerZone.packetProxy.sendFairyMount(this, ridingEntity);
    ...
    public ResourceLocation getTexture(int skin) {
        ...
        if (getCustomName().equals("Steve")) {
            texturePath = "textures/entity/notFairy.png";
        } else {
            final int idx;
            if (skin < 0) {
                idx = 1;
            ...
            texturePath = "textures/entity/fairy" + (queen() ? "q" : "") + idx + ".png";
        }
        return new ResourceLocation(Tags.MODID, texturePath);
    }
    ...
    public void setFishEntity(FairyFishHookInstance fishEntity) {
    ...
    public boolean getCanSpawnHere() {
        if (super.getCanSpawnHere()) {
            ...
            if (biome != null && (biome.rootHeight - biome.heightVariation > -0.25F)
                ...
                && biome.rainfall > 0.0F
                && biome.rainfall <= 0.8F) {
                List list = worldObj.getEntitiesWithinAABB(FairyInstance.class, this.boundingBox.expand(32D, 32D, 32D));
