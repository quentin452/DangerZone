...
public class FairyJob {
    ...
    public FairyJob(final FairyInstance entityfairy) {
        fairy = entityfairy;
    }

    public void discover(final World world) {
        if (fairy.getEntityToAttack() != null || fairy.getEntityFear() != null
            || fairy.getCryTime() > 0
            || fairy.attackTime > 0
            || fairy.getHealth() <= 0) {
            return;
    ...
    public ArrayList getAnimals(final World world) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (fairy.peacefulAnimal(entity1) && fairy.canEntityBeSeen(entity1)
                && entity1.getHealth() > 0
                && entity1.getEntityToAttack() == null
                && fleeingTick <= 0
                && !entity1.isInLove()
    ...
    public ArrayList getSheep(final World world) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            final int fleeingTick = ReflectionHelper.getPrivateValue(EntityCreature.class, entity1, MCP_FLEEINGTICK);
            if (fairy.canEntityBeSeen(entity1) && entity1.getHealth() > 0
                && entity1.getEntityToAttack() == null
                && fleeingTick <= 0
                && entity1.getGrowingAge() >= 0
                && !entity1.getSheared()) {
    ...
    private boolean checkChestItem(IInventory chest, int slot, final int x, final int y, final int z,
        final World world) {
    ...
    private boolean miscActions(final TileEntityChest chest, final int x, final int y, final int z, final World world) {
    ...
    private void cleanSlot(final TileEntityChest chest, final int p) {
        if (chest.getStackInSlot(p) != null && chest.getStackInSlot(p)
            .getItem() == null) {
    ...
    private boolean onShearingUse(final ItemStack stack, final World world) {
        ...
        for (Object one_sheep_raw : sheep) {
            ...
            if (!one_sheep.getSheared()) {
                ...
                one_sheep.entityDropItem(
                    new ItemStack(Item.getItemFromBlock(Blocks.wool), 1, one_sheep.getFleeceColor()),
                    0.0F);
    ...
    private boolean onFishingUse(final ItemStack stack, final int x, final int y, final int z, final World world) {
        ...
        final float angle = fairy.rotationYaw - 30F
            + (fairy.getRNG()
                .nextFloat() * 60F);
    ...
    private boolean trimExcessLeaves(int x, int y, int z, final World world) {
        for (int d = 0; d < 3; d++) {
            final int a = fairy.getRNG()
                .nextInt(3);
            final int b = (fairy.getRNG()
                .nextInt(2) * 2) - 1;
    ...
    private int getEmptySpace(final TileEntityChest chest, final ItemStack stack) {
        ...
        for (int i = 0; i < chest.getSizeInventory(); i++) {
            ...
            if (temp < 0 && (stack2 == null || stack2.stackSize == 0)) {
                temp = i;
            } else if (stack2 != null && stack.getItem() == stack2.getItem()
                && stack2.stackSize > 0
                && stack2.stackSize + stack.stackSize <= stack.getMaxStackSize()
                && !stack2.isItemDamaged()
                && !stack.isItemDamaged()
    ...
    private boolean isHoeItem(final Item item) {
        return item == Items.wooden_hoe || item == Items.stone_hoe
            || item == Items.iron_hoe
    ...
    private boolean isSeedItem(final Item item) {
        return item instanceof IPlantable || item == Items.reeds;
    }
    ...
    private boolean goodItem(final Item item, final int j) {
        return isHoeItem(item) || isSeedItem(item)
            || isBonemealItem(item, j)
            || isAxeItem(item)
            || isSaplingBlock(item)
            || isLogBlock(item)
            || fairy.acceptableFoods(item)
            || /* isBreedingItem( item ) || */ isShearingItem(item)
            || isClothBlock(item)
            || isFishingItem(item)
            || isRawFish(item)
            || isFlower(item);
    ...
    private final FairyInstance fairy;
