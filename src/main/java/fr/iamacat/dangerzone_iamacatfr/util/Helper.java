...
public class Helper {
    ...
    private static NetworkHelper helper = new NetworkHelper(
        "GW_DM",
        new Class[] { KnockBackPacket.class, ConfusionPacket.class });
    ...
    public static void makePlayerConfused(EntityClientPlayerMP player) {
        if (!confusedClientPlayers.contains(player)) confusedClientPlayers.add(player);
    }
    ...
    public static void makePlayerNormal(EntityClientPlayerMP player) {
        if (confusedClientPlayers.contains(player)) confusedClientPlayers.remove(player);
    }
    ...
    public static int getDifficulty(World w) {
        if (w.difficultySetting == EnumDifficulty.PEACEFUL) return 0;
        if (w.difficultySetting == EnumDifficulty.EASY) return 1;
        if (w.difficultySetting == EnumDifficulty.NORMAL) return 2;
        if (w.difficultySetting == EnumDifficulty.HARD) return 3;
