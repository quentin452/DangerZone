...
public class FairyRenderer extends RenderLiving {
    ...
    protected static ResourceLocation getRes(String key) {
        if (!resMap.containsKey(key)) {
            final ResourceLocation res = new ResourceLocation(Tags.MODID, "textures/entity/" + key + ".png");
            resMap.put(key, res);
            return res;
        } else {
    ...
    public FairyRenderer(FairyModel modelfairy, float f) {
        super(modelfairy, f);
        fairyModel = modelfairy;
        fairyModel2 = new FairyModelProps();
        fairyModel3 = new FairyModelEyes();
        fairyModel4 = new FairyModel(0.015625F);
        fairyModel5 = new FairyModelProps2();
    }

    @Override
    protected void preRenderCallback(EntityLivingBase entityliving, float f) {
        FairyInstance fairy = (FairyInstance) entityliving;
        float f1 = 0.875F;
    ...
    protected int shouldRenderPass(EntityLivingBase entityliving, int i, float f) {
        final FairyInstance fairy = (FairyInstance) entityliving;
    ...
    protected void passSpecialRender(EntityLivingBase entityliving, double d, double d1, double d2) {
        renderFairyName((FairyInstance) entityliving, d, d1, d2);
    }

    protected void renderFairyName(FairyInstance fairy, double d, double d1, double d2) {
    ...
    protected void renderLivingLabel(EntityLivingBase par1EntityLiving, String par2Str, double par3, double par5,
        double par7, float range) {
        ...
        fontrenderer.drawString(
            par2Str,
            -fontrenderer.getStringWidth(StringUtils.stripControlCodes(par2Str)) / 2,
            byte0,
            0x20ffffff);
        GL11.glEnable(GL11.GL_DEPTH_TEST);
        GL11.glDepthMask(true);
        fontrenderer
            .drawString(par2Str, -fontrenderer.getStringWidth(StringUtils.stripControlCodes(par2Str)) / 2, byte0, -1);
    ...
    protected ResourceLocation getEntityTexture(Entity entity) {
        final FairyInstance fairy = (FairyInstance) entity;
        return getEntityTexture(fairy);
    }

    protected ResourceLocation getEntityTexture(FairyInstance fairy) {
        return fairy.getTexture(fairy.getSkin());
    }

    protected FairyModel fairyModel, fairyModel4; // Body and withered overlay
    protected FairyModelProps fairyModel2; // Clothes and stuff
    protected FairyModelEyes fairyModel3; // Eyes
    protected FairyModelProps2 fairyModel5; // Rogue Clothes
