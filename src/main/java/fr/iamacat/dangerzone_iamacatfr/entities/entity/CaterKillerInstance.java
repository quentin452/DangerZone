...
public class CaterKillerInstance extends EntityMob {
    ...
    public CaterKillerInstance(final World par1World) {
        super(par1World);
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ItemStack is = null;
        this.dropItemRand(ItemInitDangerZone.CaterKillerJaw, 1);
        ...
        for (int i = 1 + this.worldObj.rand.nextInt(5), var4 = 0; var4 < i; ++var4) {
            ...
            switch (var5) {
                ...
                case 1: {
                    is = this.dropItemRand(ItemInitDangerZone.RubyIngot, 1);
                    break;
                }
                case 2: {
    ...
    private boolean scan_it(final int x, final int y, final int z, final int dx, final int dy, final int dz) {
        ...
        for (int i = -dy; i <= dy; ++i) {
            for (int j = -dz; j <= dz; ++j) {
                ...
                if (bid == Blocks.leaves || bid == Blocks.vine
                    || bid == Blocks.log
                    || bid == BlockInitDangerZone.BlockDuplicatorLog
                    || bid == Blocks.log2
                    || bid == Blocks.leaves2
    ...
    protected void updateAITasks() {
            ...
            if (this.ticker > 2400) {
                ...
                for (int i = 0; i < 10; ++i) {
                    spawnCreature(
                        ...
                        "Butterfly",
                        this.posX,
                        this.posY + 1.0 + this.worldObj.rand.nextInt(4),
                        this.posZ);
                }
                this.setDead();
                return;
        ...
        if (this.isInWeb) {
            for (int i = -2; i <= 2; ++i) {
                for (int j = -1; j < 5; ++j) {
                    for (int k = -2; k <= 2; ++k) {
                        if (this.worldObj.getBlock((int) this.posX + i, (int) this.posY + j, (int) this.posZ + k)
                            == Blocks.web) {
                            this.worldObj
                                .setBlock((int) this.posX + i, (int) this.posY + j, (int) this.posZ + k, Blocks.air);
       ...
       /* if (((this.worldObj.rand.nextInt(8) == 0 && this.getHealth() < this.mygetMaxHealth())
            || this.worldObj.rand.nextInt(30) == 0) && DangerZone.PlayNicely == 0) {
            this.closest = 99999;
            final int tx = 0;
            this.tz = tx;
            this.ty = tx;
    ...
    private boolean isSuitableTarget(final EntityLivingBase par1EntityLiving, final boolean par2) {
        ...
        if (par1EntityLiving instanceof CaterKillerInstance) {
            return false;
        }
    ...
    private EntityLivingBase findSomethingToAttack() {
        final List var5 = this.worldObj
            .getEntitiesWithinAABB(EntityLivingBase.class, this.boundingBox.expand(20.0, 8.0, 20.0));
    ...
    public boolean getCanSpawnHere() {
        ...
        for (int k = -1; k < 2; ++k) {
            for (int j = -1; j < 2; ++j) {
                for (int i = 1; i < 5; ++i) {
                    final Block bid = this.worldObj
                        .getBlock((int) this.posX + j, (int) this.posY + i, (int) this.posZ + k);
                    if (bid != Blocks.air && bid != Blocks.leaves
                        && bid != Blocks.leaves2
                        && bid != Blocks.log
                        && bid != Blocks.log2) {
                        return false;
        ...
        CaterKillerInstance target = null;
        target = (CaterKillerInstance) this.worldObj
            .findNearestEntityWithinAABB(CaterKillerInstance.class, this.boundingBox.expand(48.0, 16.0, 48.0), this);
        return target == null;
    }
