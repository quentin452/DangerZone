...
public class FairyFishHookInstance extends Entity {
    ...
    public boolean gotBite() {
        if ((ticksCatchable > 0 || onGround || ticksInAir > 1200) && rand.nextInt(2) == 0) {
            ticksCatchable -= rand.nextInt(5);
            return true;
        }
    ...
    public FairyFishHookInstance(World par1World) {
        super(par1World);
        this.xTile = -1;
        this.yTile = -1;
        this.zTile = -1;
    ...
    public FairyFishHookInstance(World par1World, double par2, double par4, double par6) {
        this(par1World);
        this.setPosition(par2, par4, par6);
        this.ignoreFrustumCheck = true;
    }

    public FairyFishHookInstance(World par1World, FairyInstance fairy) {
        super(par1World);
        this.xTile = -1;
        this.yTile = -1;
        this.zTile = -1;
        ...
        this.setLocationAndAngles(
            fairy.posX,
            fairy.posY + 1.62D - (double) fairy.yOffset,
            fairy.posZ,
            fairy.rotationYaw,
            0.25F);
    ...
    public void onUpdate() {
        if (ridingEntity != null && ridingEntity instanceof FairyInstance) // Client sided only
        {
            ridingEntity.riddenByEntity = null;
            ((FairyInstance) ridingEntity).fishEntity = this;
            this.angler = (FairyInstance) ridingEntity;
        ...
        if (this.fishPosRotationIncrements > 0) {
            ...
            if (!this.worldObj.isRemote) {
                if (this.angler == null || !this.angler.isEntityAlive()
                    || this.angler.getEntityToAttack() != null
                    || this.angler.getEntityFear() != null
                    || !this.angler.posted()
                    || !this.angler.tamed()
                    || !this.angler.isSitting()
                    || this.angler.getFishEntity() == null
            ...
            if (!this.inGround) {
                ...
                for (int var29 = 0; var29 < var27; ++var29) {
                    ...
                    double var16 = this.boundingBox.minY
                        + (this.boundingBox.maxY - this.boundingBox.minY) * (double) (var29 + 1) / (double) var27
                        - 0.125D
                        + 0.125D;
                    AxisAlignedBB var18 = AxisAlignedBB.getBoundingBox(
