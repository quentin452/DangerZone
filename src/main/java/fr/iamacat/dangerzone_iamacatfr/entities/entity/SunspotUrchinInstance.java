...
public class SunspotUrchinInstance extends EntityThrowable {
    ...
    public SunspotUrchinInstance(final World world, final EntityLivingBase entityLivingBase) {
    ...
    public SunspotUrchinInstance(final World world) {
        super(world);
        this.my_rotation = 0.0f;
        this.my_index = 81;
        this.is_special = 0;
    ...
    public SunspotUrchinInstance(final World world, final EntityLivingBase entityLivingBase, final int par3) {
        super(world, entityLivingBase);
        this.my_rotation = 0.0f;
        this.my_index = 81;
        this.is_special = 0;
    ...
    public SunspotUrchinInstance(final World world, final double par2, final double par4, final double par6) {
    ...
    protected void onImpact(final MovingObjectPosition par1MovingObjectPosition) {
        ...
        if (par1MovingObjectPosition.entityHit != null) {
            if (isAcid != 0) {
                ...
                // Mettre les entités en feu
                if (par1MovingObjectPosition.entityHit != null && this.is_iceball == 0) {
                    par1MovingObjectPosition.entityHit.setFire(5); // Réglez la durée du feu en ticks ici (5 dans cet
                                                                   // exemple)
                }

                // todo fix can't make fire on the ground
                // Créer du feu au sol
                if (!this.worldObj.isRemote) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        double offsetX = this.worldObj.rand.nextGaussian() / 10.0;
                        double offsetY = this.worldObj.rand.nextGaussian() / 10.0;
                        double offsetZ = this.worldObj.rand.nextGaussian() / 10.0;

                        // Réglez la vitesse des particules pour les rendre plus visibles
                        double motionX = 0.0;
                        double motionY = -0.1; // Réglez la vitesse verticale des particules
                        double motionZ = 0.0;

                        this.worldObj
                            .spawnParticle("flame", x + offsetX, y + offsetY, z + offsetZ, motionX, motionY, motionZ);
                    }
                }

                assert par1MovingObjectPosition.entityHit != null;
