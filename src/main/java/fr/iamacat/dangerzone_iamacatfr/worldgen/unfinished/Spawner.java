...
public final class Spawner {
    ...
    public final int doSpecificSpawning(World worldObj, Class myClass, int max, EnumCreatureType enumcreaturetype) {
        ...
        while (iterator.hasNext()) {
            ...
            ChunkPosition chunkpos = getRandomSpawningPointInChunk(
                worldObj,
                var10.chunkXPos * 16,
                var10.chunkZPos * 16);
                ...
                for (int var21 = 0; var21 < 3; ++var21) {
                    ...
                    for (int var26 = 0; var26 < 4; ++var26) {
                        ...
                        if (canCreatureTypeSpawnAtLocation(enumcreaturetype, worldObj, tempPosX, tempPosY, tempPosZ)) {
                            ...
                            if (worldObj
                                .getClosestPlayer((double) finalPosX, (double) finalPosY, (double) finalPosZ, 24.0D)
                                == null) {
                                ...
                                if (sqDist >= 576.0F) {
                                    ...
                                    entityliving.setLocationAndAngles(
                                        ...
                                        (double) finalPosY,
                                        (double) finalPosZ,
                                        worldObj.rand.nextFloat() * 360.0F,
                                        0.0F);
        ...
        if (!spawnMobs && !spawnAnmls) {
            ...
            for (int var37 = 0; var37 < var6; ++var37) {
                ...
                if ((!enumcreaturetype.getPeacefulCreature() || spawnAnmls)
                    && (enumcreaturetype.getPeacefulCreature() || spawnMobs)
                    && (enumC < getMax(enumcreaturetype))) // *
                                                           ...
                                                           // 256))
                {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        BiomeGenBase biomegenbase = worldObj.getWorldChunkManager()
                            .getBiomeGenAt(var10.getCenterXPos(), var10.getCenterZPosition()); // gets the kind of
                                                                                               // biome the
                                                                                               // chunk is at
                                                                                               // i.e. forest,
                                                                                               // etc
                        List listspawns = getCustomBiomeSpawnList(
                            getCustomSpawnableList(enumcreaturetype),
                            biomegenbase);
    ...
    private boolean canCreatureTypeSpawnAtLocation(EnumCreatureType enumcreaturetype, World world, int i, int j,
        ...
        if (enumcreaturetype.getCreatureMaterial() == Material.water) {
            ...
            return block.getMaterial()
                .isLiquid() && !block.isNormalCube();
        } else {
            final Block block = world.getBlock(i, j, k);
            final Block blockAbove = world.getBlock(i, j + 1, k);
            final Block blockBelow = world.getBlock(i, j - 1, k);

            return blockBelow.isNormalCube() && !block.isNormalCube()
                && !block.getMaterial()
                    .isLiquid()
    ...
    protected final int entityDespawnCheck(World worldObj, EntityLiving entityliving) {
        ...
        if (entityliving instanceof FairyInstance && !((FairyInstance) entityliving).canDespawn()) {
    ...
    public final int countEntities(Class class1, World worldObj) {
        ...
        for (int j = 0; j < worldObj.loadedEntityList.size(); j++) {
            ...
            if (entity instanceof FairyInstance) {
                FairyInstance fairy = (FairyInstance) entity;
