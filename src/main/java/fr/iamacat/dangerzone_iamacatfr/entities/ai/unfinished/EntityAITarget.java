...
public abstract class EntityAITarget extends EntityAIBase
{
    ...
    public boolean continueExecuting() {
        ...
        if (this.shouldCheckSight) {
            if (this.taskOwner.getEntitySenses().canSee((Entity)attackTarget)) {
                this.field_75298_g = 0;
            }
    ...
    protected double getFollowDistance() {
        final IAttributeInstance attributeinstance = this.taskOwner.getEntityAttribute(SharedMonsterAttributes.followRange);
    ...
    protected boolean isSuitableTarget(final EntityLivingBase par1EntityLiving, final boolean par2) {
        ...
        else if (this.taskOwner instanceof EntityGenericTameable && ((EntityGenericTameable)this.taskOwner).isTamed()) {
            if (par1EntityLiving instanceof EntityGenericTameable && ((EntityGenericTameable)par1EntityLiving).isTamed()) {
                return false;
            }
        ...
        if (this.taskOwner instanceof EntityGenericCreature && !((EntityGenericCreature)this.taskOwner).isWithinHomeDistance(MathHelper.floor_double(par1EntityLiving.posX), MathHelper.floor_double(par1EntityLiving.posY), MathHelper.floor_double(par1EntityLiving.posZ))) {
            return false;
        }
        if (this.taskOwner instanceof EntityCreature && !((EntityCreature)this.taskOwner).isWithinHomeDistance(MathHelper.floor_double(par1EntityLiving.posX), MathHelper.floor_double(par1EntityLiving.posY), MathHelper.floor_double(par1EntityLiving.posZ))) {
            return false;
        }
    ...
    private boolean canEasilyReach(final EntityLivingBase par1EntityLiving) {
        ...
        final PathEntity var2 = this.taskOwner.getNavigator().getPathToEntityLiving((Entity)par1EntityLiving);
        if (var2 == null) {
            return false;
        }
